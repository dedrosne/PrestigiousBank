...
namespace PrestigiousBank
{
    public class BankMenu
    {
        ...
        public virtual void RegisterBankMenu(CampaignGameStarter campaignGameStarter, Bank bank)
        {
            ...
            // Bank Menu -> Compte
            campaignGameStarter.AddGameMenuOption(String.Format("{0}_bank_menu", _cityName), String.Format("{0}_account", _cityName), "Accéder au compte",
                a => { a.optionLeaveType = GameMenuOption.LeaveType.OpenStash; return true; },
                _ => GameMenu.SwitchToMenu(String.Format("{0}_account", _cityName)),
                isLeave: false);
            ...
            //Quitter la banque
            campaignGameStarter.AddGameMenuOption(String.Format("{0}_bank_menu", _cityName), String.Format("{0}_bank_menu_back", _cityName), "Quitter la banque",
        ...
        {
            // Compte -> Dépot
            campaignGameStarter.AddGameMenuOption(String.Format("{0}_account", _cityName), String.Format("{0}_bank_deposit", _cityName), "Déposer de l'argent",
                a => { a.optionLeaveType = GameMenuOption.LeaveType.Ransom; return true; },
                _ => GameMenu.SwitchToMenu(String.Format("{0}_bank_deposit", _cityName)),
                isLeave: false);
            RegisterDepositMenuOptions(campaignGameStarter);
            // Compte -> Retrait
            campaignGameStarter.AddGameMenuOption(String.Format("{0}_account", _cityName), String.Format("{0}_bank_withdraw", _cityName), "Retirer de l'argent",
        ...
        private void RegisterDepositMenuOptions(CampaignGameStarter campaignGameStarter)
        {
            ...
            {
                campaignGameStarter.AddGameMenuOption(String.Format("{0}_bank_deposit", _cityName), 
                    String.Format("{0}_bank_deposit_{1}",_cityName, qty), 
                    String.Format("Déposer {0} pièces", qty),
                    a => {
            ...
            //Deposit All
            campaignGameStarter.AddGameMenuOption(String.Format("{0}_bank_deposit", _cityName), String.Format("{0}_bank_deposit_all", _cityName), "Tout déposer",
                    a => {
                        a.optionLeaveType = GameMenuOption.LeaveType.BribeAndEscape;
                    ...
                    i, isRepeatable: true);
            //Leave
            campaignGameStarter.AddGameMenuOption(String.Format("{0}_bank_deposit", _cityName), String.Format("{0}_bank_deposit_back", _cityName), "Retour",
                a => { a.optionLeaveType = GameMenuOption.LeaveType.Leave; return true; },
                _ => GameMenu.SwitchToMenu(String.Format("{0}_account", _cityName)),
        ...
        private void DepositGold(int amount, CampaignGameStarter campaignGameStarter)
        {
            ...
            GameMenu.SwitchToMenu(String.Format("{0}_bank_deposit", _cityName));
            CreateOrUpdateGameMenuDesc(campaignGameStarter);
        ...
        private void RegisterWithdrawMenuOptions(CampaignGameStarter campaignGameStarter)
        {
            ...
            foreach (int qty in qties)
            {
                campaignGameStarter.AddGameMenuOption(String.Format("{0}_bank_withdraw", _cityName), 
                    String.Format("{0}_bank_withdraw_{1}",_cityName, qty), 
                    String.Format("Retirer {0} pièces", qty),
                    a => {
            ...
            campaignGameStarter.AddGameMenuOption(String.Format("{0}_bank_withdraw", _cityName), String.Format("{0}_bank_withdraw_all", _cityName), "Tout retirer",
                    ...
                    i, true);
            //Leave
            campaignGameStarter.AddGameMenuOption(String.Format("{0}_bank_withdraw", _cityName), String.Format("{0}_bank_withdraw_back", _cityName), "Retour",
                a => { a.optionLeaveType = GameMenuOption.LeaveType.Leave; return true; },
                _ => GameMenu.SwitchToMenu(String.Format("{0}_account", _cityName)),
                isLeave: true, 6);
        }
